...
- `/frontend` — React 19 + TypeScript SPA (Vite). Routing via `react-router` v7; component styles via CSS Modules. Editor is CodeMirror 6 + `y-codemirror.next` bound to a `Y.Doc` synced via `y-websocket`.
  ...
  - `src/lib/` — `crdt.ts` (Yjs/y-websocket wiring), `markdown-live.ts` (CodeMirror live-preview decorations), `shortcuts.ts` (global keymap), `use-resolve.ts` (client side of the md-vs-asset rule), `tree.ts` (vault listing client), `config.ts`, `paths.ts`, `wikilink.ts`, `upload-path.ts`, `pending-rename.ts`, `cm-highlight.ts`, `vim-pref.ts` (the persisted vim-mode toggle).
...
- **No client-side database.** This is the deliberate departure from SilverBullet. Do not introduce IndexedDB/SQLite-in-browser/PouchDB/etc. on the frontend. The frontend holds the current CRDT doc in memory and nothing more.
  - **One narrow exception: editor UI preferences in `localStorage`.** The rule is about never shadowing vault/document state on the client — not about a single toggle. The vim-mode on/off flag lives at `localStorage['mdshards:vim']` (see `frontend/src/lib/vim-pref.ts`); its loss only reverts the editor to the default keymap. Keep this category to genuine UI prefs (small, non-vault, disposable). Do **not** let it grow into caching note content, the tree, or any vault-derived data — that's the database this rule forbids.
...
- Keep the frontend stateless beyond the live CRDT doc. No persistent client storage — except small, non-vault editor UI preferences (e.g. the vim-mode flag; see the "No client-side database" exception above).
