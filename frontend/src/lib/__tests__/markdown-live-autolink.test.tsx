...
describe('autolinking bare URLs and emails', () => {
  ...
  it('does NOT double-linkify the url of a [label](url) link (demo: inert)', () => {
    mountWith('intro\n\n[docs](https://example.com/x)\n')
    // DEMO divergence: an EXTERNAL `[label](url)` renders inert — the label
    // only, as `cm-md-link-external` with no `data-href`, so nothing opens.
    // The point this test still guards is the autolinker: it must not turn the
    // url half into a second, clickable link. Hence zero `.cm-md-link` nodes.
    // (Bare and <angle> autolinks stay clickable — see the tests above.)
    expect(links()).toHaveLength(0)
    const ext = document.querySelectorAll('.cm-md-link-external')
    expect(ext).toHaveLength(1)
    expect((ext[0] as HTMLElement).dataset.href).toBeUndefined()
    expect(ext[0].textContent).toBe('docs')
  })
