...
- Wiki-link image embeds — `![[pic.png]]` and `![[pic.png|alt]]` (Obsidian's default embed syntax). The target resolves **note-relative first** (a folder next to the note overshadows a same-named one at the vault root), with the **vault-rooted** path as an automatic fallback when the adjacent form doesn't exist — the browser's own load failure is the existence check. No shortest-unique-path search beyond that: an Obsidian vault set to "New link format: Absolute path in vault" or "Relative to file" round-trips; the default "shortest path" resolves only when the bare name lives next to the note or at the root. Non-image targets (`![[note]]` transclusion) stay raw — out of scope.
